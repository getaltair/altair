...
/// two calls still absent above, is that it reaches real handling — a
/// malformed `container_id` is refused as malformed, and an ordinary request
/// answers rather than falling through to the blanket `Unimplemented` the two
/// calls above still answer with.
#[tokio::test]
...
/// to the blanket `Unimplemented` the two calls above still answer with.
#[tokio::test]
