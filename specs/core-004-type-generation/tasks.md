...
- [x] Refactor `packages/bindings/src/index.ts` for better developer experience
...
- [x] Create `packages/bindings/src/shared-types.ts` for common type re-exports
  ...
  - **Note**: File created with placeholder structure; actual type re-exports commented out until domain types are generated by tauri-specta

### Task 7.3: Verify package.json exports

- [x] Ensure `packages/bindings/package.json` exports all TypeScript files correctly
  - **Acceptance**: TypeScript resolution works for all import patterns
  - **Files**: `packages/bindings/package.json`
  - **Changes**: Added explicit exports for guidance, knowledge, tracking, mobile, and shared-types modules
