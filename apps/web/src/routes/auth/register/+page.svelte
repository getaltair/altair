...
<svelte:head>
    <title>Create Account — Altair</title>
</svelte:head>
...
<div class="register-container">
    <div class="register-card">
        ...
        {#if form?.pending}
            <div class="pending-message" role="status">
                {form.message}
            </div>
        {:else}
            {#if form?.error}
...
<style>
    ...
    .error-message {
        ...
        border: 1px solid var(--color-error, rgba(220, 38, 38, 0.3));
    ...
    label {
        font-size: 0.875rem;
        font-weight: 500;
        color: var(--color-text-secondary, rgba(240, 240, 245, 0.7));
    }
    ...
    input:focus {
        outline: none;
        border-color: var(--color-accent, #6d5dfc);
    }

    button {
        width: 100%;
        padding: 0.625rem;
        background: var(--color-accent, #6d5dfc);
        color: white;
        border: none;
        border-radius: var(--radius-sm, 6px);
        font-size: 0.9375rem;
