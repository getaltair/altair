//! Application state management for Tracking app
//!
...
impl AppState {
    ...
    pub async fn new(config: AppConfig) -> Result<Self> {
        ...
        tracing::info!(
            "Initializing Tracking app state with log_level={}, db_path={:?}",
            config.log_level,
...
impl Drop for AppState {
    ...
    fn drop(&mut self) {
        tracing::info!("Tracking app state shutting down gracefully");
    }
}
...
impl AppState {
    ...
    pub async fn new_for_test(config: AppConfig) -> Result<Self> {
        ...
        let db_config = DatabaseConfig {
            url,
            namespace: "altair".to_string(),
            database: "tracking-test".to_string(),
            username: None,
            password: None,
        };
