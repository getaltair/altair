//! Application state management for Mobile app
//!
...
impl AppState {
    ...
    pub async fn new(config: AppConfig) -> Result<Self> {
        ...
        tracing::info!(
            "Initializing Mobile app state with log_level={}, db_path={:?}",
            config.log_level,
...
impl Drop for AppState {
    ...
    fn drop(&mut self) {
        tracing::info!("Mobile app state shutting down gracefully");
    }
}
...
impl AppState {
    ...
    pub async fn new_for_test(config: AppConfig) -> Result<Self> {
        ...
        let db_config = DatabaseConfig {
            url: format!("surrealkv://{}", config.database_path.display()),
            namespace: "altair".to_string(),
            database: "mobile-test".to_string(),
        };
