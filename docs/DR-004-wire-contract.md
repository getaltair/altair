...
2. **gRPC as the transport.** It is the standard carrier for protobuf, it is first-party in both Rust and Kotlin, and every outcome the boundary needs is stated rather than inferred — a typed answer or a status code, never a page of HTML a client has to recognise. Nothing sits between a client and the instance that has to understand it, because authentication is a validated token rather than a forward-auth proxy, per DR-005.
   - **Which of the two carries an outcome is not a matter of taste.** A status code carries transport conditions only: the instance was not reached, no credential resolved, the request named nothing valid. Everything the protocol has its own name for — a refusal, a position that can no longer be answered — travels inside the response message, and the call answers `Ok`. A submission whose every intent was refused is a successful call carrying refusals. **Flattening one of those into a status would make it indistinguishable from an unreachable instance**, at precisely the boundary where one signals and the other is silent, and that is the reading of "every outcome the boundary needs" this point does not carry.
