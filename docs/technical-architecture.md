...
> **ADHD-focused productivity ecosystem** — Three apps (Guidance, Knowledge,
> Tracking) with unified sync
...
Any S3-compatible storage provider works. Choose based on cost, latency, and
...
> 💡 **Rationale:** Mobile CPU/battery constraints make on-device embedding
> impractical. Embeddings are small (~1.5KB) and sync efficiently.
...
> ℹ️ **External AI providers are entirely optional.** Core functionality
> works without any providers configured.

For features like summarization, completion, and transcription, users can
...
When usage scales beyond solo use, add Prometheus metrics endpoint (`/metrics`).
...
> 💡 **For solo use:** Structured logs with duration fields are sufficient.
> Add formal metrics when you need dashboards or alerting.
...
altair/
├── apps/
│   ├── guidance/           # Tauri - Task management
│   │   ├── src/            # Svelte frontend
│   │   └── src-tauri/      # Tauri config (shares backend)
│   ├── knowledge/          # Tauri - PKM
...
│   ├── db/                 # SurrealDB bindings + schema
│   ├── sync/               # Change feed client + offline queue
│   ├── storage/            # S3 client + upload handling
│   └── search/             # Embeddings (core) + hybrid search
├── backend/
│   ├── src/
│   │   ├── main.rs
│   │   ├── commands/       # Tauri command handlers
│   │   ├── api/            # REST handlers (mobile/cloud)
│   │   ├── auth/           # Auth provider plugins
│   │   ├── sync/           # Sync orchestration
│   │   ├── embeddings/     # ONNX model, background processing
│   │   ├── providers/      # Optional: AI provider plugins
...
| Directory               | Purpose                                   |
| ----------------------- | ----------------------------------------- |
| `backend/src/commands/` | Tauri IPC handlers (desktop)              |
| `backend/src/api/`      | REST handlers (mobile/cloud)              |
...
Single installer with component selection:

| Component        | Size  | Install                          |
| ---------------- | ----- | -------------------------------- |
| Backend Service  | ~25MB | Always (systemd/Windows Service) |
| Minio (local S3) | ~50MB | Always (embedded)                |
| ONNX Runtime     | ~15MB | Always                           |
| Guidance         | ~10MB | Optional                         |
| Knowledge        | ~10MB | Optional                         |
| Tracking         | ~10MB | Optional                         |
