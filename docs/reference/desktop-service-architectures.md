...
Modern desktop applications employ surprisingly diverse approaches to background services, ranging from no separate daemon at all to sophisticated multi-process architectures with custom IPC protocols. **The dominant pattern is not a traditional system daemon, but rather tightly integrated child processes communicating via Unix sockets, named pipes, or JSON-RPC over stdio.**

### Key Finding

Most local-first desktop applications **do not use traditional background daemons** in the systemd/launchd sense. Instead, they rely on:
...
Obsidian Sync runs as a **core plugin within the main Electron process**—there is no separate daemon.
...
**Key insight**: When Obsidian is closed, no sync occurs. Changes queue locally until the app reopens.

---

### 1Password: Platform-native IPC with code signature verification

1Password 8 uses a **unified Rust core library** across all platforms, with the most sophisticated multi-client architecture studied.
...
**Key insight**: The desktop app verifies browser authenticity by checking code signatures before accepting connections.
...
**Key insight**: Browser implementation uses SharedWorker pattern—only one "active tab" can write to SQLite.

---

### Raycast: Native app with out-of-process Node.js extension runtime

Raycast is a **fully native macOS application** (Swift/AppKit) with extensions running in a single Node.js child process.
...
**Key insight**: "Conflicts are actually not that common in Linear"—LWW at property level avoids most issues.
...
VS Code uses the most complex multi-process architecture with separate extension hosts and language servers.
