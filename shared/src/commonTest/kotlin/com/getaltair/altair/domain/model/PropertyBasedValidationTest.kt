...
class PropertyBasedValidationTest {
    ...
    private val validEmailArb: Arb<String> =
        arbitrary {
    ...
    private val invalidEmailArb: Arb<String> =
        Arb.element(
            "plaintext",
            "missing@domain",
            "@nodomain.com",
    ...
    private val validHexColorArb: Arb<String> =
        arbitrary {
    ...
    private val invalidHexColorArb: Arb<String> =
        Arb.element(
            "red",
            "blue",
            "#GGG",
    ...
    private val invalidEnergyCostArb: Arb<Int> =
        Arb.element(
            -10,
            -1,
            0,
    ...
    private val tagNameWithSpacesArb: Arb<String> =
        arbitrary {
    ...
    @Test
    fun `User accepts any valid email format`() =
        runTest {
    ...
    @Test
    fun `User rejects all invalid email formats`() =
        runTest {
    ...
    @Test
    fun `User accepts display names within length limit`() =
        runTest {
    ...
    @Test
    fun `User rejects display names exceeding length limit`() =
        runTest {
    ...
    @Test
    fun `User accepts non-negative storage values`() =
        runTest {
    ...
    @Test
    fun `User correctly calculates storage remaining and over quota`() =
        runTest {
            checkAll(100, nonNegativeLongArb, nonNegativeLongArb) { used, quota ->
                val user = createUser(storageUsedBytes = used, storageQuotaBytes = quota)
    ...
    @Test
    fun `Quest accepts titles within length limit`() =
        runTest {
    ...
    @Test
    fun `Quest rejects titles exceeding length limit`() =
        runTest {
    ...
    @Test
    fun `Quest rejects invalid energy costs`() =
        runTest {
    ...
    @Test
    fun `Note accepts titles within length limit`() =
        runTest {
            checkAll(100, validStringArb(MAX_TITLE_LENGTH)) { title ->
    ...
    @Test
    fun `Note rejects titles exceeding length limit`() =
        runTest {
            checkAll(20, tooLongStringArb(MAX_TITLE_LENGTH)) { title ->
                assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun `Initiative accepts valid hex colors`() =
        runTest {
    ...
    @Test
    fun `Initiative rejects invalid hex colors`() =
        runTest {
    ...
    @Test
    fun `Initiative accepts null color`() =
        runTest {
    ...
    @Test
    fun `ColorValidation regex matches expected patterns`() =
        runTest {
            checkAll(100, validHexColorArb) { color ->
    ...
    @Test
    fun `ColorValidation regex rejects invalid patterns`() =
        runTest {
            checkAll(invalidHexColorArb.filterNot { it.isEmpty() }) { color ->
                assertTrue(!color.matches(ColorValidation.HEX_COLOR_REGEX))
            }
    ...
    @Test
    fun `Tag accepts names without spaces`() =
        runTest {
    ...
    @Test
    fun `Tag rejects names with spaces`() =
        runTest {
    ...
    @Test
    fun `NoteLink always prevents self-referencing for any ULID`() =
        runTest {
            repeat(50) {
    ...
    @Test
    fun `NoteLink accepts different source and target`() =
        runTest {
            repeat(50) {
    ...
    @Test
    fun `Folder always prevents self as parent for any ULID`() =
        runTest {
            repeat(50) {
    ...
    @Test
    fun `Location always prevents self as parent for any ULID`() =
        runTest {
            repeat(50) {
    ...
    @Test
    fun `Container always prevents self as parent for any ULID`() =
        runTest {
            repeat(50) {
    ...
    @Test
    fun `String fields accept exactly max length`() =
        runTest {
    ...
    @Test
    fun `String fields reject exactly max length plus one`() =
        runTest {
    ...
    @Test
    fun `Energy cost boundary values`() =
        runTest {
            // Min valid (1)
